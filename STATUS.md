...
- Confirmed locally on 2026-08-01: Xcode 26.3 builds the complete Intel app and its integration-test bundle; the built x86_64 app passes the non-GUI Sherpa routing smoke check; and a generic arm64 build preserves the Apple Silicon path. The updated GitHub CI workflow now runs the Intel app smoke check, with the new remote run pending the next push.
...
- Installed: the latest normal (non-test) x86_64 build is ad-hoc signed and verified at `/Users/apple54/Applications/FluidVoice Intel.app`; Parakeet TDT v2 (Blazing Fast - English) remains downloaded and active. The previous installed build is recoverably stored in Trash as `FluidVoice Intel-before-parity-2026-08-01.app`.
